...
async def autobot():
    ...
    if who.username:
        ...
        username = "gemoy" + (str(who.id))[5:] + "ubot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "gemoy" + (str(who.id))[6:] + str(ran) + "ubot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot(EditAdminRequest(int(BOTLOG_CHATID), f"@{username}", rights, "Assistant Gemoy"))
